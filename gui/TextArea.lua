...
local function formatText( text, xSize )
    ...
    for i = 1, #formatted do
        ...
        for match in formatted[i]:gmatch("%s*%S*") do
            ...
            if withoutColor:len() > xSize-xPos+1 then
                while withoutColor:len() > xSize do
                    local rest = xSize - newFormatted[#newFormatted]:gsub("[$&][%xRrLlMmo];", ""):len()
                    local currPos, actPos = 0, 0

                    while currPos < match:len() do
                        if not string.sub( match, currPos ):match( "^[$&][%xo];" ) then
                    ...
                    match = string.sub( match, currPos+( xSize > 1 and 1 or 0 ) )
                    withoutColor = match:gsub( "[$&][%xOo];", "" )

                    if withoutColor:len() > xSize then
                        newFormatted[#newFormatted+1] = ""
                        newFormattingOptions[#newFormattingOptions+1] = currentFormatting
                    end
                end
    ...
    for k, v in pairs(newFormatted) do
        if newFormattingOptions[k] == 2 then
            newFormatted[k] = (" "):rep( math.floor( ( xSize - v:gsub( "[&$][%xOo];", "" ):len() )/2 ) ) .. v
        elseif newFormattingOptions[k] == 3 then
            newFormatted[k] = (" "):rep(xSize - v:len()) .. v
        end
    end
    ...
    for k, v in pairs(text) do
        ...
        while #v > 0 do
            ...
            if color_match then
                ...
                formatted[#formatted][2] = formatted[#formatted][2] .. current_fg:rep( match:len() )
                formatted[#formatted][3] = formatted[#formatted][3] .. current_bg:rep( match:len() )

				v = string.sub(v, match:len() + 1)
            end
        end
    end
...
local function draw()
    ...
    for y = self.boundaries.y1+marginTop, self.boundaries.y2-marginBottom do
		...
		term.setCursorPos( self.boundaries.x1+self.overhangs.left+self.margin_left, y )

		if self.formattedText[ textPos ] then
			term.blit(
				string.sub( self.formattedText[textPos][1], textX1, lineLength+textX1-1 ),
				string.sub( self.formattedText[textPos][2], textX1, lineLength+textX1-1 ),
				string.sub( self.formattedText[textPos][3], textX1, lineLength+textX1-1 )
			)
		end
	end
